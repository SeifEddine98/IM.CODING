  ...
  onFileSelected(event: Event) {
    ...
    onSubmit() {
      ...
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
        data: {
          title: 'Confirmation',
          message: 'Êtes-vous sûr de vouloir ajouter cet administrateur ?'
        }
      });
    
      dialogRef.afterClosed().subscribe(result => {
        if (result) {
          this.administrateurService.addAdmin(this.photo_profile,this.administrateur).subscribe(result => {
            console.log(result);
